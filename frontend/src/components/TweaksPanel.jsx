...
// tweaks-panel — reusable Tweaks shell + form-control helpers.
// Lifted from the original CDN setup; behaves identically.
...
export function useTweaks(defaults) {
  const [values, setValues] = React.useState(() => {
    try { const v = localStorage.getItem('jobify_tweaks'); return v ? { ...defaults, ...JSON.parse(v) } : defaults; }
  ...
  const setTweak = React.useCallback((keyOrEdits, val) => {
    ...
    setValues((prev) => {
      ...
      try { localStorage.setItem('jobify_tweaks', JSON.stringify(next)); } catch {}
      return next;
    });
  }, []);
    ...
    return (
      <button
        ...
        style={{
          ...
          padding: '8px 14px', borderRadius: 999, fontSize: 11, fontFamily: 'monospace',
          background: 'rgba(250,249,247,.85)', color: '#29261b',
          border: '.5px solid rgba(0,0,0,.1)', cursor: 'pointer',
  ...
  return (
    <>
      ...
      <div ref={dragRef} className="twk-panel" style={{ right: offsetRef.current.x, bottom: offsetRef.current.y }}>
        <div className="twk-hd"><b>{title}</b><button className="twk-x" onClick={() => setOpen(false)}>✕</button></div>
